...
/**
 * Sprint L PR-L2 — coleção de view-components do tournament detail.
 *
 * Migrado da page.tsx monolítica de 3243 linhas. Cada sub-rota (chave,
 * inscritos, operacoes) importa o View que precisa daqui. Refatoração
 * completa pra primitivos (Modal/Banner/etc) é PR-L3+.
 *
 * Exports principais:
 * - `BracketView` — usado em `/torneios/:id/chave`
 * - `EntriesView` — usado em `/torneios/:id/inscritos`
 * - `OperacoesView` — usado em `/torneios/:id/operacoes`
 */
...
// ─── Bracket view ───────────────────────────────────────────────────────
// Sprint O batch O-4 — extraído pra `./_bracket.tsx`. Re-export mantém
// API pública estável pros consumers em `chave/page.tsx`.
export { BracketView } from './_bracket';

// ─── Entries view ───────────────────────────────────────────────────────
// Sprint O batch O-3 — extraído pra `./_entries.tsx`. Re-export mantém
// API pública estável pros consumers em `inscritos/page.tsx`.
