export interface MembershipRequestApprovedTemplateInput {
  klubName: string;
  klubSlug: string;
  appBaseUrl: string;
}

export function renderMembershipRequestApprovedEmail(
  input: MembershipRequestApprovedTemplateInput,
): { subject: string; html: string; text: string } {
  const dashboardUrl = `${input.appBaseUrl}/k/${input.klubSlug}/dashboard`;
  const subject = `Bem-vindo ao ${input.klubName}!`;

  const text = [
    `Olá!`,
    ``,
    `Sua solicitação de entrada no ${input.klubName} foi aprovada.`,
    `Você já pode acessar o Klub e começar a usar.`,
    ``,
        ...
        <table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e5e5e5;">
          <tr>
            <td style="padding:32px 32px 8px 32px;">
              <p style="margin:0;font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#16a34a;">Solicitação aprovada</p>
              <h1 style="margin:8px 0 0 0;font-size:22px;font-weight:700;line-height:1.3;letter-spacing:-0.01em;">Bem-vindo ao ${escapeHtml(input.klubName)} 🎾</h1>
            </td>
          </tr>
          <tr>
            <td style="padding:16px 32px 8px 32px;font-size:14.5px;line-height:1.6;color:#404040;">
              <p style="margin:0 0 12px 0;">Sua solicitação de entrada foi aprovada. Você já pode acessar o Klub, ver agenda, reservar quadras e participar das modalidades.</p>
            </td>
          </tr>
          <tr>
